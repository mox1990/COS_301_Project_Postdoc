...
package com.softserve.auxillary.util;
...
public class ApplicationServicesUtil {
    ...
    public List<Application> loadPendingApplications(Person user, String applicationStatusGroup, int StartIndex, int maxNumberOfRecords)
    {
        ...
        if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_SUBMITTED))
        {   
        ...
        else if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_REFERRED))
        {
        ...
        else if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FINALISED))
        {
            Department userDepartment = user.getEmployeeInformation().getDepartment();
            List<Application> tempOutput = new ArrayList<Application>(); 
            output = dAOFactory.createApplicationReviewRequestDAO().findAllApplicationsRequestForPersonAs(user, com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_HOD);
            for(Application application : output)
            {
                if(application.getStatus().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FINALISED))
                {
            ...
            if(output.size() < 1)
            {
                ...
                for(Application application: temp)
                {
                    ...
                    {
                        if(reviewRequest.getApplicationReviewRequestPK().getType().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_HOD))
                        {                            
                            found = true;
                            break;
                        }
        ...
        else if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_RECOMMENDED))
        {
            Faculty userFaculty = user.getEmployeeInformation().getDepartment().getFaculty();
            List<Application> tempOutput = new ArrayList<Application>(); 
            output = dAOFactory.createApplicationReviewRequestDAO().findAllApplicationsRequestForPersonAs(user, com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_DEAN);
            for(Application application : output)
            {
                if(application.getStatus().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_RECOMMENDED))
                {
                    tempOutput.add(application);
                }
            }
            ...
            if(output.size() < 1)
            {
                ...
                for(Application application: temp)
                {
                    ...
                    for(ApplicationReviewRequest reviewRequest : application.getApplicationReviewRequestList())
                    {
                        if(reviewRequest.getApplicationReviewRequestPK().getType().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_DEAN))
                        {                            
                            found = true;
                            break;
                        }
    ...
    public int getTotalNumberOfPendingApplications(Person user, String applicationStatusGroup)
    {
        ...
        else if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FINALISED))
        {
            Department userDepartment = user.getEmployeeInformation().getDepartment();
            List<Application> tempOutput = new ArrayList<Application>(); 
            List<Application> Output = getDAOFactory().createApplicationReviewRequestDAO().findAllApplicationsRequestForPersonAs(user, com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_HOD);
            for(Application application : Output)
            {
                if(application.getStatus().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FINALISED))
                {
        ...
        else if(applicationStatusGroup.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_RECOMMENDED))
        {
            Faculty userFaculty = user.getEmployeeInformation().getDepartment().getFaculty();
            List<Application> tempOutput = new ArrayList<Application>(); 
            List<Application> Output = getDAOFactory().createApplicationReviewRequestDAO().findAllApplicationsRequestForPersonAs(user, com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_REVIEW_TYPE_DEAN);
            for(Application application : Output)
            {
                if(application.getStatus().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_RECOMMENDED))
                {
    ...
    public void declineAppliction(Session session, Application application, String reason) throws AuthenticationException, NonexistentEntityException, RollbackFailureException, Exception
    {
        ...
        if(!application.getStatus().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_DECLINED))
        {
            //Deny application
            application.setStatus(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_DECLINED);        
    ...
    public void submitApplication(Application application) throws Exception
    {
        ...
        if(application.getType().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_TYPE_RENEWAL) || !application.getFundingType().equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_FUNDINGTYPE_UPPOSTDOC) || application.getRefereeReportList().size() == application.getPersonList().size())
        {
    ...
    {
        if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_OPEN))
        {
            return 0;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_SUBMITTED))
        {
            return 1;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_REFERRED))
        {
            return 2;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FINALISED))
        {
            return 3;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_RECOMMENDED))
        {
            return 4;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_ENDORSED))
        {
            return 5;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_ELIGIBLE))
        {
            return 6;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_DECLINED))
        {
            return 7;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_FUNDED))
        {
            return 8;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_COMPLETED))
        {
            return 9;
        }
        else if(status.equals(com.softserve.auxillary.constants.PersistenceConstants.APPLICATION_STATUS_TERMINATED))
        {
            return 10;
        }
